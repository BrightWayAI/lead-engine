# OpenAI portability contract — Lead Engine
...
- ChatGPT desktop: enable the plugin in a chat, then ask naturally or use `@Lead Engine <request>`.
...
Apollo, LinkedIn, CRM, mail, and calendar access are optional connectors. Ask for pasted data or operate on local pipeline state when absent. Never simulate a connector or auto-send outreach.
